...
- `Time` is replaced with $"hour" = (frac("Time", 3600, style: "horizontal")) mod 24$ to capture the diurnal signal.$"hour" = (frac("Time", 3600, style: "horizontal")) mod 24$ to capture the diurnal signal.
...
ROC-AUC is still reported for comparability, but it is not informative in this context, as it does not reflect the operational constraints of the review team. The abscissa of the ROC curve divides false positives by all negative. This means that a model with a high ROC-AUC may still generate an unmanageable number of false positives for the review team.

== Data split

Before any other operation, the dataset is split temporally into day 1 (144,236 rows, 272 frauds) and day 2 (139,490 rows, 201 frauds). Day 1 is used for hyperparameter search, model selection and threshold choice; day 2 is used only for the final evaluation of the selected model.
...
== Threshold, calibration and interpretation

To make the ranking of a model a decision, we need to decide on a threshold. The scores are sorted once by descending score and the sweep keeps one entry per distinct score, thus collapsing tied blocks. Frauds caught, queue length, and all other metrics are then prefix sums at those boundaries, so the full sweeps costs one $O(n log n)$ sort and a constant number of linear passes instead of a rescan per threshold.
...
Every unfeasible request is flagged rather than silently approximated and, because $c_"review"$ is an assumption, the optimum is recomputed across $c_"review" in {1, ..., 100}$ every run.
...
As we anticipated, ROC-AUC and PR-AUC disagree on the order and scale of the ranking. Excluding the ROC-AUC spans 0.9211-0.9777 while PR-AUC spans 0.2306-0.8783, worse than that the isolation forest scores an higher ROC-AUC than the weighted LightGBM but has a third of the PR-AUC. Ranking on ROC-AUC would ship an unusable model over a working one.
...
So the order of the top five rows of @fig:leaderboard is not supported by the data, and the pipeline to ship has to be chosen on something else.
...
From the table we can see that the number of reviews we can handle changes everything, with only 100 alerts a day every alert is fraud but we catch only 36.8% of it, thus costing us 5.2 times the best point. We can also see that on the second day, with the same threshold, the recall stays high but we waste much more work thus increasing the cost by 3.7 times. Also, neither of the way of moving the threshold to day 2 is ideal, but one is much better: keeping the old alert rate costs 8% more than the day-2 optimum, while keeping the old threshold costs 59% more.
...
The main limitation is the extreme scarcity of frauds: 473 in total, 272 of them trainable. This is why the CV is repeated, why the fold spread is quoted with every mean, why the significance test needs a variance correction, why the search bought variance reduction over capacity, and why calibration is sigmoid. Other limitations are the: number of days, two days give only one temporal split; anonymised features rule out domain checks and make the expensive misses undiagnosable; the cost model is an assumption, pricing a miss at the full amount and a review at a flat 3, ignoring recovery, chargeback fees and wrongly-blocked customers. Finally, the unsupervised branch is under-explored, @carcillo2021combining finds value in combining outlier scores with a supervised model rather than choosing, which was not tested here. Verification latency is ignored entirely.

= Reproducibility

The full source code and instructions to reproduce the run are in the `needle` repository @carella2026needle. All the experiments described were run on a AMD Ryzen 5 5600X and 16 GiB RAM. The dataset can be downloaded from the Kaggle @ulb2018dataset.

= Conclusions

At a 0.167% positive rate ROC-AUC cannot separate a deployable ranker from an unusable one, it varied eleven times less than PR-AUC across seventeen candidates, inverted their order outright, and improved across the split on which precision fell sevenfold. A tuned LightGBM over SMOTE reached 0.878 ± 0.035 in CV and 0.764 on the holdout, but the corrected paired test could not separate it from four challengers where the uncorrected test would have separated four, so it ships on fit cost, and the resampling strategy proved worth thirty times more than the learner. Carried to day 2 the threshold held recall and lost precision from 0.46 to 0.07, with day 2's own optimum at 217 alerts against 2,551 raised, so alert volume rather than a probability cut-off is the quantity to specify.
